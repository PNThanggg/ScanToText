...
class MyPrintDocumentAdapter(
    ...
    override fun onLayout(
        ...
        Timber.tag("page_dimensions").d("Page has width: $pageWidth and height: $pageHeight")
        ...
        Timber.tag("pagination").d("Pagination created!")

        totalPages = pagination.getNumberOfPages()
        Timber.tag("total_pages").d("Total pages: $totalPages")
        ...
        if (totalPages > 0) {
            ...
            Timber.tag("on_layout").e("Page count is zero.")
    ...
    private fun drawPage(
        page: PdfDocument.Page, pageNumber: Int
    ) {
        Timber.d("Writing page number: $pageNumber")
        ...
        canvas.drawText(
            titleOfDocument, leftMargin.toFloat(), titleBaseLine.toFloat(), paint
        )
    ...
    private fun Canvas.drawMultilineText(
        textPaint: TextPaint, x: Float, y: Float, width: Int, page: Int
    ) {

        Timber.tag("draw_multiline_text").d("Printing page: $page!")
        val staticLayout = update(
            page, textPaint, width
        )
    ...
    private fun update(index: Int, textPaint: TextPaint, width: Int): StaticLayout {
        ...
        Timber.tag("update").d("Text for printing: %s", text)
        return StaticLayout(
            text!!,
            0,
            text.length,
