    ...
    private fun layout() {
        ...
        for (i in 0 until lines) {
            if (height < layout.getLineBottom(i)) {
                ...
                // remove from height the new line where title fits
                height = layout.getLineTop(i) + mHeight - 35
            }
            if (i == lines - 1) {
                // Put the rest of the text into the last page
                addPage(text.subSequence(startOffset, layout.getLineEnd(i)))
                Timber.tag("pagination_finished")
                    .d("Pagination finished! Number of pages: ${getNumberOfPages()}")
                return
            }
        }
    }
