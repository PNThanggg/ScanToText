...
import com.hoicham.orc.pdf_export.PdfExportServiceImpl
...
class PdfDialogFragment : DialogFragment() {
    ...
    private val pdfExportService: PdfExportServiceImpl = PdfExportServiceImpl()
        ...
        return ComposeView(requireContext()).apply {
            ...
            setContent {
                AppTheme {
                    ...
                    PdfDialog(colorList = colorOptionsList,
    ...
    private fun createExport(scan: Scan?, color: String, fontSize: String) = scan?.let {
        pdfExportService.printDocument(
            requireContext(), it.scanTitle, listOf(
                it
            ), determineColor(color), fontSize.toInt()
        )
    }
